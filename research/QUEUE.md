...
- [x] **agent-tools/tba-census.mjs** — population-level TBA deployment/funding scan. JSON-RPC batching across 1,128 agents. First full scan: 0% TBAs deployed, 0% funded, 0% Tool Pass bonded — entire ecosystem at Level 0 (awakened only). Saves timestamped snapshots for trend tracking. See `src/agent-tools/tba-census.mjs`.
- [x] **agent-tools/normie-dossier.mjs** — comprehensive identity dossier. Combines identity, TBA status, readiness score, assets, persona, pixel history, and census context. One command, full picture. `--batch` and `--json`. See `src/agent-tools/normie-dossier.mjs`.
- [x] **agent-tools/census-diff.mjs** — deep snapshot comparison with operator fleet changes, type distribution shifts, concentration trends, and timeline mode. No API calls. See `src/agent-tools/census-diff.mjs`.
- [x] **build: normie-activate.mjs** — step-by-step activation orchestrator. Chains readiness → deploy TBAs → fund → bond Tool Pass. Dry-run default, --live to execute, --skip-bond for safety. See `src/agent-tools/normie-activate.mjs`.
- [x] **agent-tools/normie-events.mjs** — on-chain event scanner for Normies ecosystem. Queries Adapter8004 AgentBound events (awakenings) + Normies Transfer events. Block range or `--since` date, `--type` filter, `--save`, `--json`. First scan: 68 events in 24h (64 transfers, 4 awakenings, 2 burns). See `src/agent-tools/normie-events.mjs`.
- [x] **agent-tools/watchlist.mjs** — track a set of Normies and detect state changes over time. Manages a watchlist file + timestamped snapshots. Diffs on-chain state (ownership, TBA deployment, Tool Pass bonds, ETH balances, persona) between snapshots. Retry with exponential backoff for Infura rate limits. Tested on #7593, #294, #3837, #9524. See `src/agent-tools/watchlist.mjs`.
...
- [x] **agent-tools/wallet-report.mjs** — complete portfolio view for any operator. Composes fleet-view + readiness-check + activation-planner. `--deep` for cost estimates, `--json` for machine-readable. Tested on treasury wallet (#7593 = 3/7, $4.02 to full autonomy). See `src/agent-tools/wallet-report.mjs`.
- [x] **agent-tools/activation-planner.mjs** — full activation cost estimator. Live gas prices, ordered execution plan, USD estimates. Tested: #7593 = 4 steps, ~$4 total. See `src/agent-tools/activation-planner.mjs`.
...
- 2026-06-01 — `agent-tools/wallet-report.mjs` shipped. Complete portfolio view: fleet discovery + readiness checks + activation costs in one command. Tested on treasury (#7593 = 3/7, $4.02 to full autonomy).
- 2026-06-02 — `agent-tools/tba-census.mjs` shipped. First population-level TBA scan: 1,128 agents, 0% TBAs deployed, 0% funded, 0% Tool Pass bonded. Entire ecosystem at Level 0. JSON-RPC batching for efficiency.
- 2026-06-02 — `agent-tools/normie-dossier.mjs` shipped. Comprehensive identity profile: identity + TBA + readiness (7-check) + assets + persona + pixel history + census context. Tested on #7593 (3/7 configured, operator rank #309/473).
- 2026-06-02 — `agent-tools/normie-activate.mjs` shipped. Step-by-step activation orchestrator: chains deploy TBAs → fund → bond Tool Pass. Dry-run + live modes, --skip-bond, --step for single-step, batch support. Tested on #7593 (4 steps required).
